...
Both are testable, both turn out to be more interesting than they sound, and one of them is not
...
Narrower in the direction that gets a plan approved, which is worth noticing.
...
The obvious way to do this is wrong, and it is worth seeing why before seeing the right way.
...
There is one subtlety in there that is easy to skip and then be quietly wrong about. The method
works by correlating normal scores, and the rank correlation that comes out is not the number that
went in — it is attenuated, by a known amount. Declare a correlation and, uncorrected, you get a
slightly weaker one. Small, consistent, and exactly the kind of error that survives review
forever because nobody expects the number they typed to be a different number. So the relation is
inverted before use:
...
observability model's ingest chain it made it *substantially* narrower — because that chain has
several inputs feeding off the same growth, and pretending they are strangers lets them cancel
each other out.

One row barely moves, and it is there deliberately. A correlation between two inputs that do not
both feed the output in question changes almost nothing, and seeing a correlation that does not
matter beside one that does is the fastest way to stop treating the whole subject as magic.
...
That experiment does not work, and the reason it does not work is the most useful thing in this
chapter.
...
Two columns, two behaviours. The first settles. The second falls, at close to the square root of
ten per decade — the law, measured rather than asserted.
...
the regime the square-root law describes. It stays because it is the clearest demonstration of
...
The practical consequence is the arithmetic you actually need. To halve the wobble, take four
times as many samples. To get it down by a factor of ten, take a hundred times as many. That is
brutal, and it is why nobody buys precision this way past a point.

And it gives a definition of "enough" that is a calculation rather than a habit:
...
It is entirely possible — and this is the trap — to run a model at a million samples, watch the
interval stabilise to four significant figures, and present it with complete confidence, while a
...
discovers a correlation, and nothing here warns you about one you failed to declare — and an
undeclared correlation is indistinguishable, from inside the model, from a correlation of zero.
...
**That more samples are ever the answer to a wide interval.** They are not, and the first section
of this chapter is why. A wide interval means the inputs are uncertain. The only things that
narrow it are measuring something or deciding something, which is [ch19](#which-input-is-the-answer).
...
[ch20](#the-missing-node) is the failure neither chapter can see.
