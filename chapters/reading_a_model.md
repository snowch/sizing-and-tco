...
organisations. The argument for not doing so is narrow and it is worth stating precisely, because
"spreadsheets are bad" is not an argument and is not true.
...
taken. Each node declares a kind, and the kind is a statement about what sort of thing it is:
...
A value or a distribution, a unit, and a provenance: what kind of claim this is, and its source.
Both are required and the build fails without them. Three kinds, and the gap between the first two
is the one that costs money — `fact`, `vendor_claim`, `assumption`
([ch03](#where-the-numbers-come-from) is about the distinction).
...
A formula over other nodes. The unit is **checked** against what the formula produces, not
assumed — and this is where the build earns its place.

Checking dimensions alone is not enough, which took one real error to discover. Cost per terabyte
per year and cost per terabyte per month have identical dimensions and differ by a factor of
twelve. A check that compared only dimensionality would have waved the storage model's unit cost
through, twelve times too large, looking entirely plausible. So the build converts as well as
checks:
...
the graph on its own. [Appendix F](#appendix-f-observability-model) has one live on a published
page.
...
inequality next to it. So is the reason, because a margin nobody can argue with gets copied into
the next model by somebody who does not know what it was for.

Notice that the headroom here is not a number but the *name of another node* — the same one the
sizing formula uses. That is deliberate. Writing the same fraction in both places is how a model
comes to be sized for one margin and audited against another, six months after anybody remembers
there were two.
...
input so hard to spot downstream. And there is a lot of dotted border, which is the honest
summary of any model built before anybody has measured anything.

### The classification the book turns on
...
That is the front matter's central claim, arranged so that the repository enforces it. A thesis
the build does not check is a paragraph.
...
**What the right shape for a distribution is.** The DSL lets you declare four, refuses a node that
declares two, and has no opinion at all about which one belongs on a given input. That is an
...
probability out of it. The build will refuse it three different ways before it accepts it; each
refusal is a chapter.
