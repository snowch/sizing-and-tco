...
from sizing.dsl import DECIDED_BY, Ceiling, Input, discover, load_scenario, scenarios_for
...
def test_the_web_service_accounting_adds_up():
    """The one arithmetic identity worth asserting directly: a total is its parts."""
    model = MODELS["web_service"]
    values = point(model, _scenario(model, "reference"))
    assert values["capex"] == pytest.approx(values["host_capex"] + values["network_capex"])
    assert values["annual_opex"] == pytest.approx(
        values["annual_energy_cost"]
        + values["annual_licences"]
...
def test_every_input_says_who_settles_it():
    """A model cannot work out which of its numbers were somebody's choice, so it declares it.

    The nearest thing to a signal is whether an input was given a shape, and ch02 is a whole
    section on how poor a proxy that is: nothing in that chapter's file has one, so the growth
    rate reads as a decision. Left to the inference, this model filed the records its users
    uploaded as a choice, and a year in seconds as one too.
    """
    for model in discover():
        for name, node in sorted(model.nodes.items()):
            if not isinstance(node, Input):
                continue
            assert node.decided in DECIDED_BY, (
                f"{model.name}: input {name!r} says decided={node.decided!r}"
            )
...
def test_a_definition_is_never_a_choice_and_never_has_a_shape():
    ...
    A year is 365.25 days because that is what a year is. Sampling one, or offering it on a
    slider as though somebody could pick another, is a category error the units would not catch.
    """
    for model in discover():
        for name, node in sorted(model.nodes.items()):
            if isinstance(node, Input) and node.decided == "definition":
                assert not node.is_uncertain, f"{model.name}: {name!r} is sampled"
                assert node.slider is None, f"{model.name}: {name!r} is on a slider"


def test_what_the_fleet_costs_does_not_depend_on_what_it_holds():
    """The five-year total prices the fleet somebody decided to buy, not the one recommended.

    This surprises people, and it is the model's argument rather than a gap in it: sizing
    produces a recommendation and then a person decides, so doubling the data doubles what the
    model recommends and leaves the bill alone. What moves instead is the verdict — the disk and
    the cache go over their limits, which is the model saying the data will not fit on the fleet
    that was bought. If this test ever fails, the model has started buying hosts by itself and
    the ceilings have gone quiet.
    """
    model = next(m for m in discover() if m.name == "web_service")
    reach, frontier = set(), {"stored_data_t0"}
    while frontier:
        nxt = {
            other
            for other, node in model.nodes.items()
            if other not in reach and other != "stored_data_t0" and node.depends_on() & frontier
        }
        reach |= nxt
        frontier = nxt

    assert "hosts_recommended" in reach, "the data still sizes the fleet"
    assert {"disk_fill", "cache_fill"} <= reach, "and still moves the ceilings that judge it"
    assert "tco" not in reach, (
