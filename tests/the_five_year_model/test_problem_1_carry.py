...
from tests.the_five_year_model.stubs import price_from_storage_model
...
def reference():
    ...
    ).samples["cost_per_usable_tb_month"]


@pytest.mark.problem
def test_it_is_the_whole_distribution(reference):
    mine = np.asarray(price_from_storage_model(), dtype=float)
...
def test_it_is_the_right_quantity(reference):
    ...
    assert np.allclose(np.sort(mine), np.sort(reference), rtol=1e-9), (
        "these are not the same values. Read cost_per_usable_tb_month out of the storage model's "
...
def test_it_is_wide_enough_to_matter(reference):
    ...
    low, high = np.percentile(np.asarray(price_from_storage_model(), dtype=float), [5, 95])
...
def test_the_two_models_are_talking_about_the_same_unit():
    ...
    """
    upstream = load_model("models/storage_cluster/model.yaml").nodes["cost_per_usable_tb_month"]
