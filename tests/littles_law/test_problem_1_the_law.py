...
MODEL = "models/service_tier/model.yaml"
SCENARIO = "models/service_tier/scenarios/reference.yaml"
...
def test_it_agrees_with_the_model_at_the_point(evaluated):
    ...
    assert concurrency(values["arrival_rate"], values["residence_time"]) == pytest.approx(
...
def test_it_agrees_across_every_sample(evaluated):
    ...
    mine = concurrency(evaluated.samples["arrival_rate"], evaluated.samples["residence_time"])
...
def test_the_model_computes_concurrency_independently():
    ...
    assert {"arrival_rate", "residence_time"} <= node.depends_on()
